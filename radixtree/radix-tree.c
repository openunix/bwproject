...
*/
uint32_t bwlib_radix_tree_extend(struct bwlib_radix_tree_root *root, uint32_t index)
{
	struct bwlib_radix_tree_node *node;
	uint32_t height;
...
*/
uint32_t bwlib_radix_tree_insert(struct bwlib_radix_tree_root *root,
			uint32_t index, void *item)
{
	struct bwlib_radix_tree_node *node = NULL, *slot;
	...
	while (height > 0) 
	{
		if (slot == NULL)  
		{
			...
			if (!(slot = (bwlib_radix_tree_node*)malloc(sizeof(struct bwlib_radix_tree_node))))
			{
		...
		node = slot;
		slot =(bwlib_radix_tree_node*) node->slots[offset];
	...
	{
		root->rnode = (bwlib_radix_tree_node*)item; 
	}

	return 0;
}

void* bwlib_radix_tree_lookup(struct bwlib_radix_tree_root *root, uint32_t index)
{
	...
	uint32_t shift =0;
	 ...
	 {
		 bwlib_radix_tree_node * node =root->rnode;
		 while(height>0)
		 {
			 uint32_t offset =0;
			 shift = (height-1) * RADIX_TREE_MAP_SHIFT;
			 offset = (index >> shift) & RADIX_TREE_MAP_MASK;
			 node = (bwlib_radix_tree_node*)node->slots[offset];
			 shift -= RADIX_TREE_MAP_SHIFT;
			 height --;
		 }
		 return node;
...
void* bwlib_radix_tree_delete(struct bwlib_radix_tree_root *root, uint32_t index)
{
	uint32_t height = 0;
	uint32_t shift =0;
	if (index > bwlib_radix_tree_maxindex(root->height)) 
	...
	height = root->height;
	 ...
	 {
		 bwlib_radix_tree_node * node =root->rnode;
		 bwlib_radix_tree_node * parent_node =NULL;
		 uint32_t offset =0;
		 while(height>0)
		 {
			 shift = (height-1) * RADIX_TREE_MAP_SHIFT;
